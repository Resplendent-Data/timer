...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      className={cn(
        "flex h-10 w-full items-center justify-between whitespace-nowrap rounded-lg border border-input bg-input/35 px-3 py-2 text-sm shadow-sm placeholder:text-muted-foreground focus:outline-none focus-visible:ring-[3px] focus-visible:ring-ring/30 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1",
        className
      )}
      {...props}
    >
...
function SelectLabel({
  ...
  return (
    <SelectPrimitive.Label
      ...
      className={cn("px-2 py-1.5 text-xs font-bold uppercase tracking-[0.08em]", className)}
      {...props}
    />
  )
}
